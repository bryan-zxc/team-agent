...
export type ChatViewProps = {
  ...
  onInterrupt?: () => void;
  readOnly?: boolean;
  onFirstMessage?: (blocks: ContentBlock[], mentions: string[]) => void;
  initialMessage?: { blocks: ContentBlock[]; mentions: string[] };
...
export function ChatView({
  ...
  const { messages, sendMessage, sendTyping, isConnected, setMessages } = useWebSocket(
    chatId,
    memberId,
    onRoomEvent,
    handleTypingEvent,
  ...
  // Auto-send initial message once connected (fires once via ref guard)
  useEffect(() => {
    if (initialMessage && isConnected && !initialSentRef.current) {
      initialSentRef.current = true;
      sendMessage(initialMessage.blocks, initialMessage.mentions);
    }
  }, [initialMessage, isConnected, sendMessage]);
  ...
  const handleSend = useCallback(() => {
    ...
    // If no chat exists yet, delegate to onFirstMessage callback
    if (!chatId && onFirstMessage) {
      onFirstMessage(blocks, mentions);
      ri.clear();
      setMentionState(null);
      setCommandState(null);
      return;
